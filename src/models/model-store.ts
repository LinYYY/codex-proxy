...
const TRAILING_CONTEXT_SUFFIX = /\[1m\]$/i;

function sanitizeModelInput(input: string): string {
  return input.trim().replace(TRAILING_CONTEXT_SUFFIX, "").trim();
}
...
function stripKnownModelSuffixes(input: string): {
  ...
  let remaining = sanitizeModelInput(input);
...
export class ModelStore {
  ...
  resolveModelId(input: string): string {
    const trimmed = sanitizeModelInput(input);
