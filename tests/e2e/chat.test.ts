...
describe("E2E: POST /v1/chat/completions", () => {
  ...
  it("model suffix: codex-high[1m] strips context marker before routing", async () => {
    setTransportPost(async () =>
      makeTransportResponse(buildTextStreamChunks("resp_chat_alias_ctx", "Alias context suffix!")),
    );

    const res = await chatRequest(defaultBody({ model: "codex-high[1m]" }));
    expect(res.status).toBe(200);
